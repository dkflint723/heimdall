...
/// still installs. It simply stops being able to tell that Heimdall is running,
...
public class InstallerMutexTests
{
    ...
    {
        var iss = Path.Combine(RepositoryRoot().FullName, "packaging", "heimdall.iss");

        Assert.True(File.Exists(iss), $"packaging/heimdall.iss not found at {iss}");
        ...
        Assert.True(defined.Success, "heimdall.iss no longer defines an AppMutex symbol");

        Assert.Equal(Heimdall.Ui.Program.InstanceMutexName, defined.Groups[1].Value);
