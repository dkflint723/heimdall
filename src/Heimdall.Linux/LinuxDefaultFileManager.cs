...
/// <summary>
/// Makes Heimdall the desktop's file manager, through the MIME association that
...
internal sealed class LinuxDefaultFileManager : IDefaultFileManager
{
    ...
    private const string Entry = "heimdall.desktop";
    ...
    public DefaultChange MakeDefault()
    {
        ...
        return IsDefault()
            ? new DefaultChange(true, "Heimdall now opens folders.")
            ...
            : new DefaultChange(false,
                $"The desktop did not take the change. Heimdall's {Entry} may not "
    ...
    private static string? Run(params string[] args)
    {
        ...
        {
            Console.Error.WriteLine($"[heimdall] xdg-mime failed: {ex.Message}");
            return null;
        }
    }
}
