...
/// <summary>
/// Makes Heimdall the program a double-clicked folder opens, by redirecting the
...
internal sealed class WindowsDefaultFileManager : IDefaultFileManager
{
    ...
    /// machinery untouched.
    /// </summary>
    private const string Verb = "OpenInHeimdall";
    ...
    public string Caveat =>
        "Windows has no setting for this, so Heimdall registers itself as the "
        + "handler for folders and drives — double-clicking a folder will open "
        + "Heimdall. Win+E, the File Explorer button on the taskbar, and "
    ...
    public DefaultChange MakeDefault()
    {
        if (string.IsNullOrEmpty(_exe))
            return new DefaultChange(false, "Heimdall could not work out its own path.");
        ...
        try
        {
            foreach (var cls in Classes)
            {
                ...
                // the button twice must not record "Heimdall" as the thing to
                // restore to.
                ...
                verb.SetValue(null, "Open in Heimdall");
            ...
            return new DefaultChange(true, "Heimdall now opens folders and drives.");
        }
        catch (Exception ex)
        {
