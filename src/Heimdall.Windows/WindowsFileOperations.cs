...
public sealed class WindowsFileOperations : IFileOperations
{
    ...
    /// <summary>
    /// Fails, deliberately. See the class comment: this must never fall through
    /// to <see cref="Delete"/>.
    /// </summary>
    public IOperationHandle Trash(IReadOnlyList<string> paths)
    {
        ...
        handle.Failed(new PlatformNotSupportedException(
            "Moving to the Recycle Bin is not implemented on Windows yet. "
            + "Nothing was deleted. Use permanent delete if you are sure."));
