...
{
    private sealed record Running(Process Process, ShareSession Session);
    ...
    public Task<ShareSession> StartAsync(string path, bool writable, CancellationToken ct)
    {
        ...
        // Read-only unless explicitly asked otherwise. This opens a folder to
        // the network, so the default cannot be the permissive one.
        info.ArgumentList.Add("-v");
        info.ArgumentList.Add($"{path}::{(writable ? "rw" : "r")}");

        // No indexing, no thumbnails, no database: an ad-hoc share should not
        // leave a .hist folder behind in the user's directory.
        info.ArgumentList.Add("--no-thumb");
        ...
        _running[(Guid)session.Handle] = new Running(process, session);
    ...
    public Task StopAllAsync()
    {
        ...
        {
            if (_running.TryRemove(id, out var running)) Kill(running.Process);
        }
